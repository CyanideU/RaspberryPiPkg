...
- Follow the [basic steps for booting UEFI](#basic).
- In UEFI setup screen, go to `Device Manager`,
  `Raspberry Pi Configuration`, `HypDxe Configuration` and
  configure `System Boot Mode` to be `Boot in EL1`. After
  saving, Pi will reset itself.

System should boot to a single `cmd.exe` window.

Note: there are no built-in drivers for anything.

Note: you must configure `HypDxe` to `Boot in EL1` and not (the default) `Boot in EL2`, else Windows on Arm will not boot.
